...
| 자막 자동 선택 | 넷플릭스 자막이 한국어(또는 꺼짐)여도 영상을 열면 바로 일본어 대사가 전부 뜬다 |
...
Netflix 는 **지금 선택된 자막만** 내려받는다. 사용자가 한국어를 켜두면 일본어 트랙은
네트워크에 뜨지 않으므로 응답을 가로채는 것만으로는 잡을 수 없다 — 매번 플레이어에서
일본어를 직접 골라야 했던 이유다. 그래서 플레이어 API 로 트랙 목록을 직접 읽고, 학습
언어 트랙을 대신 골라 준다(다운로드 주소가 노출되는 트랙이면 사용자의 선택을 건드리지
않고 그것만 받아온다). 학습 화면을 끄면 원래 선택으로 되돌린다. 학습 중에는 넷플릭스
자체 자막을 감춘다 — 원문·번역 두 줄은 우리 오버레이가 전담한다.
